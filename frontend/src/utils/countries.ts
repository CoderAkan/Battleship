export interface Country {
    code: string;   // ISO 3166-1 alpha-2, e.g. 'KZ'
    name: string;   // English short name, e.g. 'Kazakhstan'
 ...
 *
 * Sorted alphabetically by name. Used by the signup form and profile
 * page country pickers. The `code` is what we persist to
 * `profiles.country`; `name` and `flag` are display-only.
 *
 * Note: this is a curated political list — kept in sync with the
 * standard. If a code needs to be added/removed, edit here directly.
 */
...
/**
 * Look up a country by code. Returns `undefined` if the code is unknown.
 * Useful for rendering a stored country code back into a flag + name.
 */
